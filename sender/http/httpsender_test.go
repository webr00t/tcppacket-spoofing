package http_test

import (
	. "github.com/onsi/ginkgo"
	. "github.com/onsi/gomega"
	"github.com/DennisDenuto/wifi-redirector/sender/http"
	"github.com/google/gopacket"
	"github.com/google/gopacket/layers"
)

var snoopedPacketData []byte = []byte{
	,
}

var testDecodeOptions = gopacket.DecodeOptions{
	SkipDecodeRecovery: true,
}
/*
172.17.81.73->173.222.254.225
50679->80
*/


var _ = Describe("Http/Httpsender", func() {
	Context("Http sender", func() {
		It("Should send a packet to the proper node", func() {
			httpSender := http.HttpInterceptor{
				Payload: "HELLO WORLD",
			}

			snoopedPacket := gopacket.NewPacket(snoopedPacketData, layers.LinkTypeEthernet, testDecodeOptions)

			err := httpSender.Intercept("en1", snoopedPacket, http.PacketSender{})

			Expect(err).To(BeNil())
		})
	})
})
